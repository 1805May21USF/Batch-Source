...
ADD CONSTRAINT FK_CUST_APP_CUSTOMER_ID
FOREIGN KEY (CUSTOMER_ID) REFERENCES JDBCBANK_CUSTOMER(CUSTOMER_ID)
ON DELETE CASCADE;

ALTER TABLE JDBCBANK_CUSTOMER_APPLICATION
ADD CONSTRAINT FK_CUST_APP_APPLICATION_ID
...
ADD CONSTRAINT FK_CUST_ACC_CUSTOMER_ID
FOREIGN KEY (CUSTOMER_ID) REFERENCES JDBCBANK_CUSTOMER(CUSTOMER_ID)
ON DELETE CASCADE;

ALTER TABLE JDBCBANK_CUSTOMER_ACCOUNT
ADD CONSTRAINT FK_CUST_ACC_ACCOUNT_ID
...
CREATE OR REPLACE PROCEDURE add_application( b IN NUMBER,a IN VARCHAR)
IS
BEGIN
    INSERT INTO JDBCBANK_APPLICATION (INITIAL_BALANCE,APPROVAL) VALUES (b,a);
    COMMIT;
END;
/

CREATE OR REPLACE PROCEDURE update_application( aid IN NUMBER,b IN NUMBER,a IN VARCHAR)
IS
BEGIN
    UPDATE JDBCBANK_APPLICATION SET INITIAL_BALANCE = b, APPROVAL = a WHERE APPLICATION_ID = aid;
    COMMIT;
END;
/
...
CREATE OR REPLACE PROCEDURE add_account(b in NUMBER, s in VARCHAR)
IS
BEGIN
    INSERT INTO JDBCBANK_ACCOUNT (BALANCE,STATUS) VALUES (b,s);
    COMMIT;
END;
/

CREATE OR REPLACE PROCEDURE update_account(aid IN NUMBER, b in NUMBER,s in VARCHAR)
IS
BEGIN
    UPDATE JDBCBANK_ACCOUNT SET BALANCE = B, STATUS = S WHERE ACCOUNT_ID = aid;
    COMMIT;
END;
/
...
SELECT * FROM JDBCBANK_EMPLOYEE;
SELECT * FROM JDBCBANK_CUSTOMER;
